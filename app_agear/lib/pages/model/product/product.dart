...
import 'package:app_agear/pages/model/product/add.dart';
import 'package:app_agear/pages/function.dart';
import 'package:app_agear/pages/model/component/progress.dart';
...
class _ProductState extends State<Product> {
  ...
  void getDataFood(int count, String strSearch) async {
    ...
    List arr = await getData(count, 20, "food/readfoodcustomer.php", strSearch,
        "cat_id=${widget.cat_id}&cus_id=${G_cus_id_val.length}&");
    for (int i = count; i < arr.length; i++) {
      foodList.add(new FoodData(
        foo_id: arr[i]["foo_id"].toString(),
        cat_id: arr[i]["cat_id"].toString(),
        foo_name: arr[i]["foo_name"],
        foo_name_en: arr[i]["foo_name_en"],
        foo_price: arr[i]["foo_price"].toString(),
        foo_offer: arr[i]["foo_offer"].toString(),
        foo_info: arr[i]["foo_info"],
        foo_info_en: arr[i]["foo_info_en"],
        fav_id: arr[i]["fav_id"].toString(),
        foo_regdate: arr[i]["foo_regdate"].toString(),
        foo_image: arr[i]["foo_thumbnail"]== null? "def.png"
            : arr[i]["foo_thumbnail"],
      ));
    }
    loadingList = false;
          ...
          Container(
            ...
            child: new Column(
              ...
              children: <Widget>[
                ...
                checkOffer(food.foo_offer.toString())
              ],
            ),
          )
          //=========================end favorite
