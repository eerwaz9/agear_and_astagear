  ...
  void getDatabill(int count, String strSearch) async {
    ...
    for (int i = 0; i < arr.length; i++) {
      billList.add(new DetailBillData(
        det_id: arr[i]["det_id"].toString(),
        foo_id: arr[i]["foo_id"].toString(),
        foo_name: arr[i]["foo_name"],
        foo_image: arr[i]["foo_image"],
        det_note: arr[i]["det_note"],
        det_price: arr[i]["det_price"].toString(),
        det_qty: arr[i]["det_qty"].toString(),
      ));
      sum += int.parse(arr[i]["det_price"].toString()) * int.parse(arr[i]["det_qty"].toString());
    }
    loadingList = false;
    setState(() {});
  }
            ...
            children: [
              Row(
                children: [
                  new Text(
                      "الاجمالي :" +
                          (int.parse(DetailBill.det_qty.toString()) *
                                  int.parse(DetailBill.det_price.toString()))
                              .toString(),
                      style: TextStyle(fontFamily: "arial", fontSize: 16)),
                  new Text("      "),
                  new Text("السعر : " + DetailBill.det_price.toString(),
                      style: TextStyle(
                          fontFamily: "arial",
                          color: Colors.orange,
                          fontSize: 16)),
                  new Text(" "),
                  new Text("الكمية :" + DetailBill.det_qty.toString(),
                      style: TextStyle(
                          fontFamily: "arial",
                          color: Colors.orange,
                          fontSize: 16)),
