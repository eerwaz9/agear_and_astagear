...
class _BillState extends State<Bill> {
  ...
  void getDatabill(int count, String strSearch) async {
    ...
    List arr = await getData(
        count, 20, "bill/readbill.php", strSearch, "cus_id=${G_cus_id_val.length}&");
    for (int i = 0; i < arr.length; i++) {
      billList.add(new BillData(
        bil_id: arr[i]["bil_id"].toString(),
        cus_id: arr[i]["cus_id"].toString(),
              ...
              Row(
                children: [
                  new Text(
                    bill.bil_id,
                    style: TextStyle(
                        fontFamily: "arial", color: Colors.orange, fontSize: 16),
                  ),
                  new Text("  "),
